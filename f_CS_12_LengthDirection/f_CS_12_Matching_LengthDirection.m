function [ OrderedMatchingResults ] = f_CS_12_Matching_LengthDirection( pointlist1, alldata, filetype, foldername, sampercen, match, fullfeatures )
...
for i = 1:mylength
    ...
    if mylength == 1400 %only work for MPEG7
        ...
        if mylength == 271 %only work for Berkely
            ...
            [csfeature1] = f_CS_12_Feature_LengthDirection(sampoint1);
            [csfeature2] = f_CS_12_Feature_LengthDirection(sampoint2);
        else
            %for sketching
    ...
    if match == 4 %the original matching algorithm
        [similarity] = f_CS_12_Similarity_Original_LengthDirection(csfeature1, csfeature2);
    end
