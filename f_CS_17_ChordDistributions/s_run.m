...
if dataset == 1 %retrieval for BerkeleyCS dataset
    for i =query:query
        ...
        [matchingresult] = f_CS_17_Matching_ChordDistributions(pointlist1, alldata, filetype, foldername, sampercen, match, fullfeatures);
        TotalResults{1,1} = query;
        TotalResults{1,2} = objectname1;
        TotalResults{1,3} = matchingresult;
        myfolder = ['resources/Re-RichCS-',matchlabel,'-',datalebel,'-ChordDistributions-',samlabel];
        if exist(myfolder, 'dir')
            save([myfolder,'/',num2str(query),'.mat'],'TotalResults');
            savestring = ['Re-RichCS-',matchlabel,'-',datalebel,'-ChordDistributions-',samlabel,'-',num2str(query),'.mat'];
        else
            mkdir(myfolder);
            save([myfolder,'/',num2str(query),'.mat'],'TotalResults');
            savestring = ['Re-RichCS-',matchlabel,'-',datalebel,'-ChordDistributions-',samlabel,'-',num2str(query),'.mat'];
...
if dataset == 2 %retrieval for SketchingCS dataset
    [SketchingList] = f_objects_sketching();
     %load the features
    if mylength == 1
        load('resources/Feature-Sketching-ChordDistributions-L1.mat');
    end
    if mylength == 2
        load('resources/Feature-Sketching-ChordDistributions-L2.mat');
    end
    if mylength == 3
        load('resources/Feature-Sketching-ChordDistributions-L3.mat');
    end
    if mylength == 4
        load('resources/Feature-Sketching-ChordDistributions-L4.mat');
    end
    ...
    save(['resources/Re-RichCS-',matchlabel,'-',datalebel,'-ChordDistributions-',samlabel,'.mat'],'TotalResults');
    savestring = ['Re-RichCS-',matchlabel,'-',datalebel,'-ChordDistributions-',samlabel,'.mat'];
end
...
%     end
    ...
    if mylength == 1
        load('resources/Feature-MPEG7-ChordDistributions-L1.mat');
    end
    if mylength == 2
        load('resources/Feature-MPEG7-ChordDistributions-L2.mat');
    end
    if mylength == 3
        load('resources/Feature-MPEG7-ChordDistributions-L3.mat');
    end
    if mylength == 4
        load('resources/Feature-MPEG7-ChordDistributions-L4.mat');
    end
