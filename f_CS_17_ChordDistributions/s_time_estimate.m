...
for i = 1:N
    ...
    [csfeature] = f_CS_17_Feature_ChordDistributions(pointlist);
    fullfeatures{i} = csfeature;
end
T_Feature = toc;
save('resources/T_Feature_ChordDistributions.mat','T_Feature');
%%
%time for retrieval
...
for i = 1:length(myquerys)
    ...
    for j = 1:mylength
        ...
        [similarity] = f_CS_17_Similarity_DTW_ChordDistributions(csfeature1, csfeature2);
    end
    mytime = toc;
    QueryDTW{i,3} = mytime;
end
save('resources/QueryDTWChordDistributions.mat','QueryDTW');
clear QueryDTW;
...
for i = 1:length(myquerys)
    ...
    for j = 1:mylength
        %display(['--->',num2str(j)]);
        csfeature2 = fullfeatures{j};
        [similarity] = f_CS_17_Similarity_DP_ChordDistributions(csfeature1, csfeature2);
    end
    mytime = toc;
    QueryDP{i,3} = mytime;
end
save('resources/QueryDPChordDistributions.mat','QueryDP');
clear QueryDP;
...
for i = 1:length(myquerys)
    ...
    for j = 1:mylength
        %display(['--->',num2str(j)]);
        csfeature2 = fullfeatures{j};
        [similarity] = f_CS_17_Similarity_Hungarian_ChordDistributions(csfeature1, csfeature2);
    end
    mytime = toc;
    QueryHungarian{i,3} = mytime;
end
save('resources/QueryHungarianChordDistributions.mat','QueryHungarian');
