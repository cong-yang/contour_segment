...
if dataset == 1 %retrieval for BerkeleyCS dataset
    for i =query:query
        ...
        [matchingresult] = f_CS_7_Matching_TurningAngles(pointlist1, alldata, filetype, foldername, sampercen, match, fullfeatures);
        ...
        myfolder = ['resources/Re-RichCS-',matchlabel,'-',datalebel,'-TurningAngles-',samlabel];
        if exist(myfolder, 'dir')
            save([myfolder,'/',num2str(query),'.mat'],'TotalResults');
            savestring = ['Re-RichCS-',matchlabel,'-',datalebel,'-TurningAngles-',samlabel,'-',num2str(query),'.mat'];
        else
            mkdir(myfolder);
            save([myfolder,'/',num2str(query),'.mat'],'TotalResults');
            savestring = ['Re-RichCS-',matchlabel,'-',datalebel,'-TurningAngles-',samlabel,'-',num2str(query),'.mat'];
...
if dataset == 2 %retrieval for SketchingCS dataset
    [SketchingList] = f_objects_sketching();
     %load the features
    if mylength == 1
        load('resources/Feature-Sketching-TurningAngles-L1.mat');
    end
    if mylength == 2
        load('resources/Feature-Sketching-TurningAngles-L2.mat');
    end
    if mylength == 3
        load('resources/Feature-Sketching-TurningAngles-L3.mat');
    end
    if mylength == 4
        load('resources/Feature-Sketching-TurningAngles-L4.mat');
    end
    ...
    save(['resources\Re-RichCS-',matchlabel,'-',datalebel,'-TurningAngles-',samlabel,'.mat'],'TotalResults');
    savestring = ['Re-RichCS-',matchlabel,'-',datalebel,'-TurningAngles-',samlabel,'.mat'];
end
...
%     end
    ...
    if mylength == 1
        load('resources/Feature-MPEG7-TurningAngles-L1.mat');
    end
    if mylength == 2
        load('resources/Feature-MPEG7-TurningAngles-L2.mat');
    end
    if mylength == 3
        load('resources/Feature-MPEG7-TurningAngles-L3.mat');
    end
    if mylength == 4
        load('resources/Feature-MPEG7-TurningAngles-L4.mat');
    end
