...
for i = 1:N
    ...
    [csfeature] = f_CS_7_Feature_TurningAngles(pointlist);
    fullfeatures{i} = csfeature;
end
T_Feature = toc;
save('resources/T_Feature_TurningAngles.mat','T_Feature');
%%
%time for retrieval
...
for i = 1:length(myquerys)
    ...
    for j = 1:mylength
        ...
        [similarity] = f_CS_7_Similarity_DTW_TurningAngles(csfeature1, csfeature2);
    end
    mytime = toc;
    QueryDTW{i,3} = mytime;
end
save('resources/QueryDTWTurningAngles.mat','QueryDTW');
clear QueryDTW;
...
for i = 1:length(myquerys)
    ...
    for j = 1:mylength
        %display(['--->',num2str(j)]);
        csfeature2 = fullfeatures{j};
        [similarity] = f_CS_7_Similarity_DP_TurningAngles(csfeature1, csfeature2);
    end
    mytime = toc;
    QueryDP{i,3} = mytime;
end
save('resources/QueryDPTurningAngles.mat','QueryDP');
clear QueryDP;
...
for i = 1:length(myquerys)
    ...
    for j = 1:mylength
        display(['--->',num2str(j)]);
        csfeature2 = fullfeatures{j};
        [similarity] = f_CS_7_Similarity_Hungarian_TurningAngles(csfeature1, csfeature2);
    end
    mytime = toc;
    QueryHungarian{i,3} = mytime;
end
save('resources/QueryHungarianTurningAngles.mat','QueryHungarian');
