function [ OrderedMatchingResults ] = f_CS_6_Matching_ContourContext( pointlist1, alldata, filetype, foldername, sampercen, match, fullfeatures )
%f_CS_5_Matching_PointTriangle: do the matching with matrix algorithm and output the 
%                               ranked final result
%   input:
...
%             end
% 
%             [csfeature1] = f_CS_6_Feature_ContourContext(sampoint1);
%             [csfeature2] = f_CS_6_Feature_ContourContext(sampoint2);
        %end
    ...
    if match == 1 %the DTW matching algorithm
        [similarity] = f_CS_6_Similarity_DTW_ContourContext(csfeature1, csfeature2);
    end
    
    if match == 2 %the DP matching algorithm
        [similarity] = f_CS_6_Similarity_DP_ContourContext(csfeature1, csfeature2);
    end
    
    if match == 3 %the hungarian matching algorithm
        [similarity] = f_CS_6_Similarity_Hungarian_ContourContext(csfeature1, csfeature2);
    end
