    ...
    if isempty(missed) == 0 %if there are some missing data
        if boolwriting
            ...
            for j = 1:length(missed)
                %sbatch --output=log/log_2_3_4_1390 --job-name=2_3_4_1390 job.slurm 2 3 4 1390
                %sbatch --output=log/log_2_3_2_1267 --job-name=2_3_2_1267 job.slurm 2 3 2 1267
                %fprintf(fid,strcat('NFMaxBoxSizeRatio','=',num2str(NFMaxBoxSizeRatio),'\r\n'));
                myline = strcat('sbatch --output=log/log_',num2str(datalable),'_',num2str(matchlabel),'_',num2str(lengthlable),'_',num2str(missed(j)),' --job-name=',num2str(datalable),'_',num2str(matchlabel),'_',num2str(lengthlable),'_',num2str(missed(j)),' job.slurm',{' '},num2str(datalable),{' '},num2str(matchlabel),{' '},num2str(lengthlable),{' '},num2str(missed(j)),'\n');
                myline = [myline{:}];
                fprintf(fid,myline);
