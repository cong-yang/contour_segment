function [ OrderedMatchingResults ] = f_CS_15_Matching_CSS( pointlist1, alldata, filetype, foldername, sampercen, match, fullfeatures )
...
for i = 1:mylength
    ...
    if mylength == 1400 %only work for MPEG7
        ...
        %else
            ...
            [csfeature1] = f_CS_15_Feature_CSS(sampoint1);
            [csfeature2] = f_CS_15_Feature_CSS(sampoint2);
        %end
    end


    if match == 1 %the DTW matching algorithm
        [similarity] = f_CS_15_Similarity_DTW_CSS(csfeature1, csfeature2);
    end
    
    if match == 2 %the DP matching algorithm
        [similarity] = f_CS_15_Similarity_DP_CSS(csfeature1, csfeature2);
    end
    
    if match == 3 %the hungarian matching algorithm
        [similarity] = f_CS_15_Similarity_Hungarian_CSS(csfeature1, csfeature2);
    end
