...
if dataset == 1 %retrieval for BerkeleyCS dataset
    for i =query:query
        ...
        [matchingresult] = f_CS_15_Matching_CSS(pointlist1, alldata, filetype, foldername, sampercen, match, fullfeatures);
        ...
        myfolder = ['resources/Re-RichCS-',matchlabel,'-',datalebel,'-CSS-',samlabel];
        if exist(myfolder, 'dir')
            save([myfolder,'/',num2str(query),'.mat'],'TotalResults');
            savestring = ['Re-RichCS-',matchlabel,'-',datalebel,'-CSS-',samlabel,'-',num2str(query),'.mat'];
        else
            mkdir(myfolder);
            save([myfolder,'/',num2str(query),'.mat'],'TotalResults');
            savestring = ['Re-RichCS-',matchlabel,'-',datalebel,'-CSS-',samlabel,'-',num2str(query),'.mat'];
...
%         fullfeatures = 0; %only for temp
%         [matchingresult] = f_CS_3_Matching_HeightFunction(pointlist1, alldata, filetype, foldername, sampercen, match, fullfeatures);
%         TotalResults{i} = {objectname1,matchingresult};
%     end
...
%     end
    ...
    if mylength == 1
        load('resources/Feature-MPEG7-CSS-L1.mat');
    end
    if mylength == 2
        load('resources/Feature-MPEG7-CSS-L2.mat');
    end
    if mylength == 3
        load('resources/Feature-MPEG7-CSS-L3.mat');
    end
    if mylength == 4
        load('resources/Feature-MPEG7-CSS-L4.mat');
    end
