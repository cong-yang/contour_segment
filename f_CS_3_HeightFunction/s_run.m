...
if dataset == 1 %retrieval for BerkeleyCS dataset
    for i =query:query
        ...
        myfolder = ['resources/Re-RichCS-',matchlabel,'-',datalebel,'-HeightFunction-',samlabel];
        if exist(myfolder, 'dir')
            save([myfolder,'/',num2str(query),'.mat'],'TotalResults');
            savestring = ['Re-RichCS-',matchlabel,'-',datalebel,'-HeightFunction-',samlabel,'-',num2str(query),'.mat'];
        else
            mkdir(myfolder);
            save([myfolder,'/',num2str(query),'.mat'],'TotalResults');
            savestring = ['Re-RichCS-',matchlabel,'-',datalebel,'-HeightFunction-',samlabel,'-',num2str(query),'.mat'];
...
%     end
    ...
    if mylength == 1
        load('resources/Feature-MPEG7-HeightFunction-L1.mat');
    end
    if mylength == 2
        load('resources/Feature-MPEG7-HeightFunction-L2.mat');
    end
    if mylength == 3
        load('resources/Feature-MPEG7-HeightFunction-L3.mat');
    end
    if mylength == 4
        load('resources/Feature-MPEG7-HeightFunction-L4.mat');
    end
