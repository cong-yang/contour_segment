...
if dataset == 1 %retrieval for BerkeleyCS dataset
    for i =query:query
        ...
        [matchingresult] = f_CS_8_Matching_BeamAngles(pointlist1, alldata, filetype, foldername, sampercen, match, fullfeatures);
        ...
        myfolder = ['resources/Re-RichCS-',matchlabel,'-',datalebel,'-BeamAngles-',samlabel];
        if exist(myfolder, 'dir')
            save([myfolder,'/',num2str(query),'.mat'],'TotalResults');
            savestring = ['Re-RichCS-',matchlabel,'-',datalebel,'-BeamAngles-',samlabel,'-',num2str(query),'.mat'];
        else
            mkdir(myfolder);
            save([myfolder,'/',num2str(query),'.mat'],'TotalResults');
            savestring = ['Re-RichCS-',matchlabel,'-',datalebel,'-BeamAngles-',samlabel,'-',num2str(query),'.mat'];
...
if dataset == 2 %retrieval for SketchingCS dataset
    [SketchingList] = f_objects_sketching();
     %load the features
    if mylength == 1
        load('resources/Feature-Sketching-BeamAngles-L1.mat');
    end
    if mylength == 2
        load('resources/Feature-Sketching-BeamAngles-L2.mat');
    end
    if mylength == 3
        load('resources/Feature-Sketching-BeamAngles-L3.mat');
    end
    if mylength == 4
        load('resources/Feature-Sketching-BeamAngles-L4.mat');
    end
    ...
    save(['resources/Re-RichCS-',matchlabel,'-',datalebel,'-BeamAngles-',samlabel,'.mat'],'TotalResults');
    savestring = ['Re-RichCS-',matchlabel,'-',datalebel,'-BeamAngles-',samlabel,'.mat'];
end
...
%     end
    ...
    if mylength == 1
        load('resources/Feature-MPEG7-BeamAngles-L1.mat');
    end
    if mylength == 2
        load('resources/Feature-MPEG7-BeamAngles-L2.mat');
    end
    if mylength == 3
        load('resources/Feature-MPEG7-BeamAngles-L3.mat');
    end
    if mylength == 4
        load('resources/Feature-MPEG7-BeamAngles-L4.mat');
    end
