...
for i = 1:N
    ...
    [csfeature] = f_CS_8_Feature_BeamAngles(pointlist);
    fullfeatures{i} = csfeature;
end
T_Feature = toc;
save('resources/T_Feature_BeamAngles.mat','T_Feature');
%%
%time for retrieval
...
for i = 1:length(myquerys)
    ...
    for j = 1:mylength
        ...
        [similarity] = f_CS_8_Similarity_DTW_BeamAngles(csfeature1, csfeature2);
    end
    mytime = toc;
    QueryDTW{i,3} = mytime;
end
save('resources/QueryDTWBeamAngles.mat','QueryDTW');
clear QueryDTW;
...
for i = 1:length(myquerys)
    ...
    for j = 1:mylength
        %display(['--->',num2str(j)]);
        csfeature2 = fullfeatures{j};
        [similarity] = f_CS_8_Similarity_DP_BeamAngles(csfeature1, csfeature2);
    end
    mytime = toc;
    QueryDP{i,3} = mytime;
end
save('resources/QueryDPBeamAngles.mat','QueryDP');
clear QueryDP;
...
for i = 1:length(myquerys)
    ...
    for j = 1:mylength
        display(['--->',num2str(j)]);
        csfeature2 = fullfeatures{j};
        [similarity] = f_CS_8_Similarity_Hungarian_BeamAngles(csfeature1, csfeature2);
    end
    mytime = toc;
    QueryHungarian{i,3} = mytime;
end
save('resources/QueryHungarianBeamAngles.mat','QueryHungarian');
