...
for i = 1:N
    ...
    [csfeature] = f_CS_11_Feature_OptPartialContours(pointlist);
    fullfeatures{i} = csfeature;
end
T_Feature = toc;
save('resources/T_Feature_OptPartialContours.mat','T_Feature');
%%
%time for retrieval
...
for i = 1:length(myquerys)
    ...
    for j = 1:mylength
        ...
        [similarity] = f_CS_11_Similarity_DTW_OptPartialContours(csfeature1, csfeature2);
    end
    mytime = toc;
    QueryDTW{i,3} = mytime;
end
save('resources/QueryDTWOptPartialContours.mat','QueryDTW');
clear QueryDTW;
...
for i = 1:length(myquerys)
    ...
    for j = 1:mylength
        %display(['--->',num2str(j)]);
        csfeature2 = fullfeatures{j};
        [similarity] = f_CS_11_Similarity_DP_OptPartialContours(csfeature1, csfeature2);
    end
    mytime = toc;
    QueryDP{i,3} = mytime;
end
save('resources/QueryDPOptPartialContours.mat','QueryDP');
clear QueryDP;
...
for i = 1:length(myquerys)
    ...
    for j = 1:mylength
        %display(['--->',num2str(j)]);
        csfeature2 = fullfeatures{j};
        [similarity] = f_CS_11_Similarity_Hungarian_OptPartialContours(csfeature1, csfeature2);
    end
    mytime = toc;
    QueryHungarian{i,3} = mytime;
end
save('resources/QueryHungarianOptPartialContours.mat','QueryHungarian');
